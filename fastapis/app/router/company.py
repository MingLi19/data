...
from app.core.security import get_admin_user, get_system_admin_user
...
@api.post("", summary="创建公司", dependencies=[Depends(get_admin_user)])
...
@api.put("/{company_id}", summary="更新公司信息", dependencies=[Depends(get_admin_user)])
async def update_company(
    company_id: int, company: CompanyUpdate, service: CompanyService = Depends(get_company_service)
) -> ResponseModel[Company]:
    company = service.update_company(company_id, company)
    return {"code": 200, "data": company, "message": "公司信息更新成功"}


@api.delete("/{company_id}", summary="删除公司", dependencies=[Depends(get_system_admin_user)])
